...
function shouldFetchData({ movie }) {
  return !movie.movies || !movie.isFetching
}
...
function fetchMovie() {
  return (dispatch, getState) => {
    if (shouldFetchData(getState())) {
      apiProps.url = 'https://api.themoviedb.org/3/movie/top_rated?api_key=9dd0443589af0dc500baa06c8b00e960&language=en-US&page=1'
...
function SearchMovie(w) {
  return (dispatch, getState) => {
    if (shouldFetchData(getState())) {
      apiProps.url = 'https://api.themoviedb.org/3/search/movie?api_key=9dd0443589af0dc500baa06c8b00e960&language=en-US&page=1&include_adult=false&query=' + w
...
function fetchOne(one) {
  return (dispatch, getState) => {
    if (shouldFetchData(getState())) {
      apiProps.url = 'https://api.themoviedb.org/3/movie/' + one + '?api_key=9dd0443589af0dc500baa06c8b00e960&language=en-US'
